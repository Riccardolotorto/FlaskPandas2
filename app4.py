...
def home():
    ...
    return render_template('ricercaCheck.html', list= list(nomi_regioni['denominazione_regione']))

@app.route('/search', methods = ['POST'])
def search():
    regioni = request.form.getlist("regione") 
    risultato = pd.DataFrame() #creo un dataframe di pandas
    for regione in regioni:
        ...
        risultato = pd.concat([risultato, data])   #concat: mette in un unico dataframe due o più dataframe uno sotto l'altro
    if len(risultato) == 0:
